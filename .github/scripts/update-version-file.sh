...
if [[ "$(curl -SsL https://api.github.com/repos/nextlinux/${BIN}/releases/tags/${VERSION} | jq .prerelease)" == "true" ]] ; then
...
aws s3 cp "${VERSION_FILE}" s3://toolbox-data.next-linux.systems/${BIN}/releases/latest/VERSION
