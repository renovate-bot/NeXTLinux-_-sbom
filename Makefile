...
unit: $(TEMP_DIR) fixtures  ## Run unit tests (with coverage)
	...
	go test -coverprofile $(TEMP_DIR)/unit-coverage-details.txt $(shell go list ./... | grep -v nextlinux/sbom/test)
...
cli: $(SNAPSHOT_DIR)  ## Run CLI tests
	...
	SBOM_BINARY_LOCATION='$(SNAPSHOT_BIN)' \
...
fingerprints:
	...
	cd sbom/pkg/cataloger/binary/test-fixtures && \
		make cache.fingerprint

	# for JAVA BUILD test fixtures
	cd sbom/pkg/cataloger/java/test-fixtures/java-builds && \
		make cache.fingerprint

	# for GO BINARY test fixtures
	cd sbom/pkg/cataloger/golang/test-fixtures/archs && \
		make binaries.fingerprint

	# for RPM test fixtures
	cd sbom/pkg/cataloger/rpm/test-fixtures && \
		make rpms.fingerprint

	# for Kernel test fixtures
	cd sbom/pkg/cataloger/kernel/test-fixtures && \
		make cache.fingerprint
...
fixtures:
	...
	cd sbom/pkg/cataloger/java/test-fixtures/java-builds && make
	cd sbom/pkg/cataloger/rpm/test-fixtures && make
	cd sbom/pkg/cataloger/binary/test-fixtures && make
...
generate-compare-file:
	...
	go run ./cmd/sbom $(COMPARE_TEST_IMAGE) -o json > $(COMPARE_DIR)/test-fixtures/acceptance-centos-8.2.2004.json
