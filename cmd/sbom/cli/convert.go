...
func Convert(v *viper.Viper, app *config.Application, ro *options.RootOptions, po *options.PackagesOptions) *cobra.Command {
	cmd := &cobra.Command{
		...
		Long:  "[Experimental] Convert SBOM files to, and from, SPDX, CycloneDX and sbom's format. For more info about data loss between formats see https://github.com/nextlinux/sbom#format-conversion-experimental",
