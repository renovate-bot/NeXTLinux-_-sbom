...
import (
	...
	golangCataloger "github.com/nextlinux/sbom/sbom/pkg/cataloger/golang"
...
// Application is the main sbom application configuration.
type Application struct {
	// the location where the application config was read from (either from -c or discovered while loading); default .sbom.yaml
...
func loadConfig(v *viper.Viper, configPath string) error {
	...
	if _, err := os.Stat("config.yaml"); err == nil {
		log.Warn("DEPRECATED: ./config.yaml as a configuration file is deprecated and will be removed as an option in v1.0.0, please rename to .sbom.yaml")
	}
