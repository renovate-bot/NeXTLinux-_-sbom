...
func TestApplicationConfig(t *testing.T) {
	...
	// VALID: .sbom.yaml, .sbom/config.yaml, ~/.sbom.yaml, <XDG_CONFIG_HOME>/sbom/config.yaml
	...
	tests := []struct {
	...
	}{
		{
			...
			setup: func(t *testing.T) string {
				return "./test-fixtures/.sbom.yaml"
		...
		{
			name: "current working directory sbom dir config",
			setup: func(t *testing.T) string {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			// this will override home in case you are running this test locally and DO have a sbom config
