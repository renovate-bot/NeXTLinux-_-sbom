...
import (
	...
	"github.com/nextlinux/sbom/sbom/formats/common"
...
func encodeComponent(p pkg.Package) cyclonedx.Component {
	props := encodeProperties(p, "sbom:package")
	props = append(props, encodeCPEs(p)...)
	locations := p.Locations.ToSlice()
	if len(locations) > 0 {
		props = append(props, encodeProperties(locations, "sbom:location")...)
	}
	if hasMetadata(p) {
		props = append(props, encodeProperties(p.Metadata, "sbom:metadata")...)
	}
...
func deriveBomRef(p pkg.Package) string {
	// try and parse the PURL if possible and append sbom id to it, to make
...
func decodeComponent(c *cyclonedx.Component) *pkg.Package {
	...
	common.DecodeInto(p, values, "sbom:package", CycloneDXFields)
...
func decodeLocations(vals map[string]string) source.LocationSet {
	v := common.Decode(reflect.TypeOf([]source.Location{}), vals, "sbom:location", CycloneDXFields)
...
func decodePackageMetadata(vals map[string]string, c *cyclonedx.Component, typ pkg.MetadataType) interface{} {
	if typ != "" && c.Properties != nil {
		...
		metaPtr := common.Decode(metaPtrTyp, vals, "sbom:metadata", CycloneDXFields)
