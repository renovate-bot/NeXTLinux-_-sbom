...
import (
	...
	sbomFile "github.com/nextlinux/sbom/sbom/file"
...
// sbom supported digests: cmd/sbom/cli/eventloop/tasks.go
// MD5, SHA1, SHA256
...
func decodeExternalReferences(c *cyclonedx.Component, metadata interface{}) {
	...
	case *pkg.GemMetadata:
		meta.Homepage = refURL(c, cyclonedx.ERTypeWebsite)
	case *pkg.JavaMetadata:
		var digests []sbomFile.Digest
		if ref := findExternalRef(c, cyclonedx.ERTypeBuildMeta); ref != nil {
			if ref.Hashes != nil {
				for _, hash := range *ref.Hashes {
					digests = append(digests, sbomFile.Digest{
						Algorithm: sbomFile.CleanDigestAlgorithmName(string(hash.Algorithm)),
						Value:     hash.Value,
					})
				}
			}
