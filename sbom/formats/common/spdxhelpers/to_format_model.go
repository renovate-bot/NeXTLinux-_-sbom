...
import (
	...
	"github.com/nextlinux/sbom/internal/spdxlicense"
...
func toPackages(catalog *pkg.Collection, sbom sbom.SBOM) (results []*spdx.Package) {
	for _, p := range catalog.Sorted() {
		...
		// 1. sbom has generated a sha1 digest for the package itself - usually in the java cataloger
		// 2. sbom has generated a sha1 digest for the package's contents
...
func toPackageChecksums(p pkg.Package) ([]spdx.Checksum, bool) {
	...
	case pkg.JavaMetadata:
		// if sbom has generated the digest here then filesAnalyzed is true
